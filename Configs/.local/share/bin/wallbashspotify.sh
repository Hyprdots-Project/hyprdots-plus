...
if pkg_installed spotify-launcher && pkg_installed spicetify-cli ; then

    # This was needed for spotify package from AUR.
    # Commenting it out for people who still wanna use the AUR version
    ...
    if [ "$(spicetify config | awk '{if ($1=="color_scheme") print $2}')" != "Wallbash" ] || [[ "${*}" == *"--reset"*  ]] ; then
	    ...
	    curl -L -o ${cacheDir}/landing/Spotify_Sleek.tar.gz https://github.com/Hyprdots-Project/hyprdots-plus/blob/main/Source/arcs/Spotify_Sleek.tar.gz
