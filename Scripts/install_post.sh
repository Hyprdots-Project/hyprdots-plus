...
if is_package_installed "sddm"; then
  ...
  if [[ ! -f "/etc/sddm.conf.d/kde_settings.t2.bkp" ]]; then
    ...
    sudo tar -xzf \
      "$CLONE_DIR/Source/arcs/Sddm_$sddm_theme.tar.gz" -C "/usr/share/sddm/themes"
    sudo touch "/etc/sddm.conf.d/kde_settings.conf"
    sudo cp \
      "/etc/sddm.conf.d/kde_settings.conf" "/etc/sddm.conf.d/kde_settings.t2.bkp"
    sudo cp \
      "/usr/share/sddm/themes/$sddm_theme/kde_settings.conf" "/etc/sddm.conf.d/"
  else
  ...
  if [[ ! -f "/usr/share/sddm/faces/$USER.face.icon" ]] \
    ...
    if sudo cp \
      "$CLONE_DIR/source/misc/$USER.face.icon" "/usr/share/sddm/faces"; then
...
if is_package_installed "dolphin" && is_package_installed "xdg-utils"; then
  # Check if dolphin is already set as default
